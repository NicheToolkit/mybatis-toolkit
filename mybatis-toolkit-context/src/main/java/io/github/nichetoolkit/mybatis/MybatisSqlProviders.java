...
public interface MybatisSqlProviders {
    ...
    static <I> String providing(ProviderContext providerContext, @Nullable String tablename, I id, MybatisSqlProviders sqlProvider) throws RestException {
        return providing(providerContext, tablename, id, table -> {}, sqlProvider);
    }

    static <I> String providing(ProviderContext providerContext, @Nullable String tablename, I id, ConsumerActuator<MybatisTable> actuator, MybatisSqlProviders sqlProvider) throws RestException {
    ...
    static <I> String providing(ProviderContext providerContext, @Nullable String tablename, Collection<I> idList, MybatisSqlProviders sqlProvider) throws RestException {
        return providing(providerContext, tablename, idList, table -> {}, sqlProvider);
    }

    static <I> String providing(ProviderContext providerContext, @Nullable String tablename, Collection<I> idList, ConsumerActuator<MybatisTable> actuator, MybatisSqlProviders sqlProvider) throws RestException {
        return MybatisSqlScript.caching(providerContext, (table, sqlScript) -> {
            actuator.actuate(table);
            Map<Integer, List<I>> identitySliceMap = Collections.emptyMap();
            if (table.isIdentity()) {
                RestStream.stream(idList).forEach(id -> valueOfIdentity(table, id));
                identitySliceMap = sliceOfIdentity(table, idList);
            }
            return sqlProvider.provide(tablename, table, identitySliceMap, sqlScript);
        });
    }

    MybatisSqlProviders ALERT_FIELD_BY_ID = new MybatisSqlProviders() {
        @Override
    ...
    MybatisSqlProviders ALERT_FIELD_ALL = new MybatisSqlProviders() {
        @Override
        public <I> String provide(@Nullable String tablename, MybatisTable table, Map<Integer, List<I>> identitySliceMap, MybatisSqlScript sqlScript) throws RestException {
    ...
    MybatisSqlProviders ALERT_BY_ID = new MybatisSqlProviders() {
        @Override
        public <I> String provide(@Nullable String tablename, MybatisTable table, Map<Integer, List<I>> identitySliceMap, MybatisSqlScript sqlScript) throws RestException {
    ...
    MybatisSqlProviders ALERT_ALL = new MybatisSqlProviders() {
        @Override
        public <I> String provide(@Nullable String tablename, MybatisTable table, Map<Integer, List<I>> identitySliceMap, MybatisSqlScript sqlScript) throws RestException {
    ...
    MybatisSqlProviders DELETE_BY_LINK_ID = new MybatisSqlProviders() {
        @Override
        public <I> String provide(@Nullable String tablename, MybatisTable table, Map<Integer, List<I>> identitySliceMap, MybatisSqlScript sqlScript) throws RestException {
            return updateOfAlertByFieldSql(tablename, table, sqlScript) + identityWhereSql(table, sqlScript);
        }
    ...
    MybatisSqlProviders DELETE_BY_LINK_IDS = new MybatisSqlProviders() {
        @Override
        public <I> String provide(@Nullable String tablename, MybatisTable table, Map<Integer, List<I>> identitySliceMap, MybatisSqlScript sqlScript) throws RestException {
            return updateOfAlertByFieldSql(tablename, table, sqlScript) + identitiesWhereSql(identitySliceMap, table, sqlScript);
        }
    ...
    static String upsetOfSaveSql(@Nullable String tablename, MybatisTable table) throws RestException {
        ...
        switch (databaseType) {
            ...
            default:
                if (isDoNothing) {
                    upsetSql = " ON CONFLICT (" + table.identityColumnSql() + ") DO NOTHING ";
                } else {
                    upsetSql = " ON CONFLICT (" + table.identityColumnSql() + ") DO UPDATE SET ";
                }
                break;
        }
    ...
    interface SimpleSqlProvider extends MybatisSqlProviders {
        ...
        <I> String provide(@Nullable String tablename, MybatisTable mybatisTable, Map<Integer, List<I>> identitySliceMap, MybatisSqlScript sqlScript, MybatisSqlProviders sqlProvider) throws RestException;
