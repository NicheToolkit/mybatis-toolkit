...
public class MybatisCaching extends XMLLanguageDriver {
    ...
    public static String cache(ProviderContext providerContext, MybatisTable entity, SupplierActuator<String> sqlScriptSupplier) throws RestException {
        ...
        if (!MybatisCacheSupport.CACHE_SQL.containsKey(cacheKey)) {
            isPresentLang(providerContext);
            synchronized (cacheKey) {
                if (!MybatisCacheSupport.CACHE_SQL.containsKey(cacheKey)) {
                    MybatisCacheSupport.CACHE_SQL.put(cacheKey, new MybatisSqlCache(
    ...
    public SqlSource createSqlSource(Configuration configuration, String script, Class<?> parameterType) {
         ...
         */
        if (MybatisCacheSupport.CACHE_SQL.containsKey(script)) {
            /* 为了容易理解，使用 cacheKey 变量代替 script */
            String cacheKey = script;
            /* 判断是否已经解析过 */
            if (!(MybatisCacheSupport.CONFIGURATION_CACHE_KEY_MAP.containsKey(configuration) && MybatisCacheSupport.CONFIGURATION_CACHE_KEY_MAP.get(configuration).containsKey(cacheKey))) {
                synchronized (cacheKey) {
                    if (!(MybatisCacheSupport.CONFIGURATION_CACHE_KEY_MAP.containsKey(configuration) && MybatisCacheSupport.CONFIGURATION_CACHE_KEY_MAP.get(configuration).containsKey(cacheKey))) {
                        /* 取出缓存的信息 */
                        MybatisSqlCache sqlCache = MybatisCacheSupport.CACHE_SQL.get(cacheKey);
                        ...
                        Map<String, SqlSource> cacheKeyMap = MybatisCacheSupport.CONFIGURATION_CACHE_KEY_MAP.computeIfAbsent(configuration, k -> new HashMap<>());
                        ...
                        /* 取消cache对象的引用，减少内存占用 */
                        if (MybatisCacheSupport.USE_ONCE) {
                            MybatisCacheSupport.CACHE_SQL.put(cacheKey, MybatisSqlCache.NULL_SQL_CACHE);
                        }
                    }
                }
            }
            return MybatisCacheSupport.CONFIGURATION_CACHE_KEY_MAP.get(configuration).get(cacheKey);
        } else {
